...
namespace TheBilet.Membership
{
    public class MembershipUtils
    {
        ...
        public static String ResetPassword(HttpContext context)
        {
            return parseArguments(context, ServiceCommand.resetPassword);
        }
        ...
        public static String parseArguments(HttpContext context, ServiceCommand command)
        {
            try
            {
                switch (command)
                {
                    ...
                    case ServiceCommand.resetPassword:
                        return Membership.ResetPassword(context.Request.Params[VAR_USERNAME].ToString(), context.Request.Params[VAR_EMAIL].ToString());
